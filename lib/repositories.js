...
function normalizeKycStatus(status) {
  ...
  if (KYC_STATUSES.includes(normalized)) return normalized;
...
function createRepositories(collections) {
  ...
  // A user can now have up to 2 active orders at once (e.g. one order stuck in
  // a dispute shouldn't stop them placing another) — a single-field unique
  // index on sellerUserId can only express "at most 1", so it's dropped here
  // rather than recreated. The "at most 2" cap itself lives in app-level
  // checks (see server.js's pre-create middleware and p2p-order-controller.js).
  async function ensureSellerActiveOrderIndex() {
    let indexes;
    try {
      indexes = await p2pOrders.indexes();
    } catch (e) {
      if (e.codeName === 'NamespaceNotFound' || (e.message && e.message.includes('ns does not exist'))) {
        indexes = [];
      } else {
        throw e;
      }
    ...
    for (const index of indexes) {
      ...
      // Drop the old max-1-active-order unique index, desired or not.
  ...
  async function ensureSignupOtpIndexes() {
    let indexes;
    try {
      indexes = await signupOtps.indexes();
    } catch (e) {
      if (e.codeName === 'NamespaceNotFound' || (e.message && e.message.includes('ns does not exist'))) {
        indexes = [];
      } else {
  ...
  async function ensureIndexes() {
    ...
    await Promise.all([
      ...
      p2pCredentials.createIndex({ userId: 1 }, { sparse: true }),
      ...
      // Belt-and-suspenders for the upsert-based race-condition fix in
      // server.js's deposit-bonus grant: even if that call site's atomicity
      // is ever weakened by a future edit, this makes a second claim row for
      // the same user physically impossible at the database level.
      ...(depositBonusClaims ? [depositBonusClaims.createIndex({ userId: 1 }, { unique: true })] : []),
      ...
      p2pOrders.createIndex({ sellerUserId: 1, createdAt: -1 }),
      p2pOrders.createIndex({ buyerUserId: 1, createdAt: -1 }),
      p2pOrders.createIndex({ 'participants.id': 1, createdAt: -1 }),
      p2pOrders.createIndex({ sellerUsername: 1, createdAt: -1 }),
      p2pOrders.createIndex({ buyerUsername: 1, createdAt: -1 }),
      p2pOrders.createIndex({ createdAt: -1 }),
      // Compound indexes for fast my-active queries (status + userId)
      p2pOrders.createIndex({ sellerUserId: 1, status: 1, updatedAt: -1 }),
      p2pOrders.createIndex({ buyerUserId: 1, status: 1, updatedAt: -1 }),
      p2pOrders.createIndex({ 'participants.id': 1, status: 1, updatedAt: -1 }),
  ...
  async function incrementSignupOtpAttempts(contact, options = {}) {
    const purpose = String(options.purpose || 'signup').trim().toLowerCase() || 'signup';
    // MongoDB driver v6+: findOneAndUpdate returns the document directly.
    return signupOtps.findOneAndUpdate(
      { contact, purpose },
      { $inc: { attempts: 1 } },
  ...
  async function setP2PCredential(email, passwordHash, options = {}) {
    ...
    const credUserId = `usr_${crypto.createHash('sha1').update(email).digest('hex').slice(0, 16)}`;
    const updateDoc = {
      email,
      userId: credUserId,
      passwordHash,
    ...
    await p2pCredentials.updateOne(
      { email },
      {
        ...
        $setOnInsert: {
          createdAt: now,
          emailVerified: true, // registration requires OTP verification, so email is already confirmed
          isMerchant: false,
  ...
  async function getP2PCredentialByUserId(userId) {
    ...
    if (!uid) return null;
    return p2pCredentials.findOne({ userId: uid });
  }
  ...
  async function touchP2PCredentialLogin(email, loginMeta = {}) {
    const updateDoc = {
      ...
      emailVerified: true // OTP login proves email ownership
    };
  ...
  async function upsertP2PKycRequest(userId, email, payload = {}) {
    ...
    const doc = {
      ...
      livenessVideo: String(payload.livenessVideo || '').trim(),
  ...
  async function updateLastActive(userId, username) {
    const orConditions = [{ createdByUserId: userId }];
    if (username && String(username).trim()) {
      orConditions.push({ advertiser: String(username).trim() });
      orConditions.push({ createdByUsername: String(username).trim() });
    }
    await p2pOffers.updateMany(
      { $or: orConditions, status: { $ne: 'DELETED' } },
      { $set: { lastActiveAt: new Date() } }
    );
  }

  async function updateOffer(offerId, userId, fields) {
    const allowed = ['price', 'minLimit', 'maxLimit', 'payments', 'status', 'remark', 'totalAmount', 'available', 'availableAmount', 'releaseTime'];
    ...
    // The offer card display reads the legacy `available` field (and the
    // next edit's escrow delta calc reads `escrowLockedAmount`) — both must
    // stay in sync with `availableAmount` or an edit silently shows/locks a
    // stale number even though the wallet-side lock was correct.
    if (fields.totalAmount) {
      $set.availableAmount = fields.totalAmount;
      $set.available = fields.totalAmount;
      $set.escrowLockedAmount = fields.totalAmount;
    }
  ...
  async function getUserTradeOrders(userId, limit = 30) {
    const rows = await tradeOrders.find({ userId }).sort({ createdAt: -1 }).limit(limit).toArray();
    return rows.map((row) => {
      const mapped = { ...row, createdAt: toDate(row.createdAt).toISOString() };
      delete mapped._id;
      return mapped;
  ...
  async function listP2POrderHistory(userId, { limit = 50, offset = 0, username = '', email = '' } = {}) {
    const uid = String(userId || '').trim();
    ...
    // Exact string match so MongoDB can use the sellerUsername/buyerUsername indexes
    if (uname) {
      orConditions.push({ sellerUsername: uname });
      orConditions.push({ buyerUsername: uname });
      orConditions.push({ 'participants.username': uname });
    }
    // Email is stored lowercase; normalizedEmail is already lowercased
    if (normalizedEmail) {
      orConditions.push({ sellerEmail: normalizedEmail });
      orConditions.push({ buyerEmail: normalizedEmail });
      orConditions.push({ 'participants.email': normalizedEmail });
    }
    const query = { $or: orConditions };
    const queryOpts = { maxTimeMS: 15000 };
    const [rows, total] = await Promise.all([
      p2pOrders.find(query, queryOpts).sort({ createdAt: -1 }).skip(safeOffset).limit(safeLimit).toArray(),
      p2pOrders.countDocuments(query, queryOpts)
    ]);
    const orders = rows.map((row) => { const m = { ...row }; delete m._id; return m; });
    return { orders, total, hasMore: safeOffset + orders.length < total };
  }

  // Fast path for /api/p2p/orders/my-active — only scans active+recent statuses
  // using the compound (userId, status) indexes instead of fetching all history.
  async function listMyActiveOrders(userId) {
    const uid = String(userId || '').trim();
    if (!uid) return [];
    const activeStatuses = ['CREATED', 'PENDING', 'PAID', 'PAYMENT_SENT', 'DISPUTED'];
    const recentCutoff = Date.now() - 24 * 60 * 60 * 1000;
    const recentStatuses = ['RELEASED', 'COMPLETED', 'CANCELLED', 'EXPIRED'];
    const statusFilter = { $in: [...activeStatuses, ...recentStatuses] };
    // Three narrow indexed queries then merge in JS — much faster than wide $or
    const [asSeller, asBuyer, asParticipant] = await Promise.all([
      p2pOrders.find({ sellerUserId: uid, status: statusFilter }, { maxTimeMS: 8000 }).sort({ updatedAt: -1 }).limit(30).toArray(),
      p2pOrders.find({ buyerUserId: uid, status: statusFilter }, { maxTimeMS: 8000 }).sort({ updatedAt: -1 }).limit(30).toArray(),
      p2pOrders.find({ 'participants.id': uid, status: statusFilter }, { maxTimeMS: 8000 }).sort({ updatedAt: -1 }).limit(30).toArray()
    ]);
    // Deduplicate by order id, prefer most recent copy
    const seen = new Map();
    for (const row of [...asSeller, ...asBuyer, ...asParticipant]) {
      if (!seen.has(row.id)) seen.set(row.id, row);
    }
    return Array.from(seen.values())
      .filter((o) =>
        activeStatuses.includes(o.status) ||
        (recentStatuses.includes(o.status) && Number(o.updatedAt || 0) > recentCutoff)
      )
      .sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0))
      .map((row) => { const m = { ...row }; delete m._id; return m; });
  }
  ...
  async function listPaymentMethods(userId) {
    const docs = await p2pPaymentMethods.find({ userId: String(userId) }).sort({ createdAt: 1 }).toArray();
    return docs.map(d => ({ id: String(d._id), ...d }));
  }
  ...
  async function addPaymentMethod(userId, data) {
    const doc = {
      ...
      provider: String(data.provider || data.nickname || data.type || ''),
      nickname: String(data.nickname || data.provider || data.type || ''),
    ...
    const result = await p2pPaymentMethods.insertOne(doc);
    return { id: String(result.insertedId), ...doc };
  }

  async function updatePaymentMethod(pmId, userId, data) {
    const { ObjectId } = require('mongodb');
    let oid;
    try { oid = new ObjectId(pmId); } catch { return null; }
    const update = {};
    const allowed = ['accountHolder','upiId','accountNumber','ifsc','bankName','bankBranch','accountType','details','qrCode','nickname'];
    allowed.forEach(k => { if (data[k] !== undefined) update[k] = String(data[k]); });
    update.updatedAt = new Date();
    const result = await p2pPaymentMethods.findOneAndUpdate(
      { _id: oid, userId: String(userId) },
      { $set: update },
      { returnDocument: 'after' }
    );
    if (!result) return null;
    return { id: String(result._id), ...result };
  }

  async function deletePaymentMethod(pmId, userId) {
    const { ObjectId } = require('mongodb');
    let oid;
    try { oid = new ObjectId(pmId); } catch { return false; }
    const result = await p2pPaymentMethods.deleteOne({ _id: oid, userId: String(userId) });
  ...
  async function getUserReputation(userId) {
    ...
    const participantQuery = {
      $or: [
        { sellerUserId: normalizedUserId },
        { sellerId: normalizedUserId },
        { buyerUserId: normalizedUserId },
        { buyerId: normalizedUserId }
      ]
    };

    const orders = await p2pOrders.find(
      participantQuery,
      {
        projection: {
          id: 1,
    ...
    const completedStatuses = new Set(['RELEASED', 'COMPLETED']);
    const cancelledStatuses = new Set(['CANCELLED', 'EXPIRED']);
    const totalOrders = orders.length;
    const completedOrders = orders.filter((order) => completedStatuses.has(String(order?.status || '').trim().toUpperCase())).length;
    const cancelledOrders = orders.filter((order) => cancelledStatuses.has(String(order?.status || '').trim().toUpperCase())).length;
    const buyOrders = orders.filter((order) => String(order?.buyerUserId || order?.buyerId || '').trim() === normalizedUserId).length;
    const sellOrders = orders.filter((order) => String(order?.sellerUserId || order?.sellerId || '').trim() === normalizedUserId).length;

    let avgStars = null;
    let ratingCount = 0;
    const orderIds = orders.map((order) => String(order?.id || '').trim()).filter(Boolean);
    if (orderIds.length) {
      const ratings = await p2pRatings.find({
        orderId: { $in: orderIds },
        raterId: { $ne: normalizedUserId }
      }).project({ stars: 1 }).toArray();
      ratingCount = ratings.length;
      if (ratingCount > 0) {
        const totalStars = ratings.reduce((sum, rating) => sum + Number(rating?.stars || 0), 0);
        avgStars = totalStars / ratingCount;
      }
    }
    ...
    return {
      ...
      avgStars: avgStars == null ? null : Number(avgStars.toFixed(1)),
      ratingCount
    };
  }
  ...
  async function blockUser(blockerId, blockedId) {
    const now = new Date();
    await p2pBlockedUsers.updateOne(
      { blockerId: String(blockerId), blockedId: String(blockedId) },
      { $set: { blockerId: String(blockerId), blockedId: String(blockedId), createdAt: now } },
      { upsert: true }
    );
  }

  async function unblockUser(blockerId, blockedId) {
    await p2pBlockedUsers.deleteOne({ blockerId: String(blockerId), blockedId: String(blockedId) });
  }

  async function getBlockedUsers(blockerId) {
    return p2pBlockedUsers.find({ blockerId: String(blockerId) }).toArray();
  }

  async function isBlockedBy(blockerId, blockedId) {
    const doc = await p2pBlockedUsers.findOne({ blockerId: String(blockerId), blockedId: String(blockedId) });
    return !!doc;
  }
}
