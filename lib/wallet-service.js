...
function createWalletService(collections, mongoClient, options = {}) {
  const { wallets, p2pOrders, p2pOffers, p2pCredentials, ledgerEntries, walletFailures, withdrawalRequests, adminUserProfiles } = collections;
  const DEFAULT_USER_WALLET_BALANCE = toAmount(process.env.P2P_DEFAULT_USER_BALANCE || 0);
      ...
      const result = await wallets.updateOne(
        {
          _id: walletDoc._id,
          $and: [
            { $or: [{ availableBalance: beforeAvailable }, { availableBalance: { $exists: false }, balance: beforeAvailable }] },
  ...
  async function setOrderDisputed(orderId, actor, options = {}) {
    ...
    const isActorParticipant =
      [order.sellerUserId, order.buyerUserId, order.sellerId, order.buyerId].includes(userId) ||
      (actorUsername && [order.sellerUsername, order.buyerUsername].some(
        (u) => String(u || '').trim().toLowerCase() === actorUsername)) ||
      (actorEmail && [order.sellerEmail, order.buyerEmail].some(
        (e) => String(e || '').trim().toLowerCase() === actorEmail));
    if (!isActorParticipant) {
      throw createAppError('FORBIDDEN', 'Only participants can raise dispute.', 403);
    }
    if (!['PENDING', 'PAID', 'PAYMENT_SENT', 'CREATED'].includes(order.status)) {
      throw createAppError('INVALID_ORDER_STATUS', 'Only active orders can be disputed. For completed orders use Order Inquiry.', 400);
    }
    ...
    await p2pOrders.updateOne(
      { id: orderId },
      {
        $set: {
          ...
          appealedByUsername: actorUsername,
          appealedByEmail: actorEmail,
  ...
  async function cancelOrder(orderId, actor, status = 'CANCELLED') {
    ...
    try {
      ...
      const result = await (async () => {
        ...
        // Admin (isSystem) dispute resolution may cancel a DISPUTED order to return escrow to the ad.
        const allowedStatuses = targetStatus === 'EXPIRED'
          ? EXPIRABLE_ORDER_STATUSES
          : isSystem
            ? [...CANCELLABLE_ORDER_STATUSES, 'DISPUTED']
        ...
        // Per-user admin override: an admin can flag a specific user (e.g. one who
        // repeatedly abuses cancellation to back out after being paid) as unable to
        // cancel an order once payment has been marked sent — same trigger point as
        // the buyer-only rule below, just scoped to one user. isSystem bypasses this
        // too, since that's the admin's own dispute-resolution path.
        if (
          !isSystem &&
          ...
          const profile = await adminUserProfiles.findOne({ userId: actorId }).catch(() => null);
          if (profile && profile.cancelDisabled === true) {
            throw createAppError(
              'FORBIDDEN',
              'Your account is restricted from cancelling orders after payment has been marked as sent. Contact support if you believe this is a mistake.',
              403
            );
          }
        }
  ...
  // Admin correction of the destination address on a withdrawal that has NOT yet been
  // processed. Atomic + validated: the update only lands while the request is still
  // PENDING, so an admin cannot silently redirect crypto that is already approved/sent.
  // Returns { previousAddress, newAddress, request }.
  async function updateWithdrawalRequestAddress(requestId, rawAddress) {
    const normalizedRequestId = String(requestId || '').trim();
    if (!normalizedRequestId) {
      throw createAppError('INVALID_WITHDRAWAL_REQUEST_ID', 'Withdrawal request id is required.');
    }
    const newAddress = ensureWithdrawalAddress(rawAddress);

    const existing = await withdrawalRequests.findOne({ requestId: normalizedRequestId });
    if (!existing) {
      throw createAppError('WITHDRAWAL_NOT_FOUND', 'Withdrawal request not found.', 404);
    }
    const currentStatus = normalizeWithdrawalStatus(existing.status || 'pending');
    if (currentStatus !== 'pending') {
      throw createAppError(
        'INVALID_WITHDRAWAL_STATE',
        'Only pending withdrawals can have their address changed.',
        409
      );
    }
    const previousAddress = existing.address || existing.toAddress || null;

    const result = await withdrawalRequests.updateOne(
      { requestId: normalizedRequestId, status: existing.status },
      { $set: { address: newAddress, toAddress: newAddress, updatedAt: new Date() } }
    );
    if (result.matchedCount === 0) {
      // Status changed underneath us between the read and the write.
      throw createAppError(
        'INVALID_WITHDRAWAL_STATE',
        'Withdrawal request changed state — address not updated.',
        409
      );
    }

    const updated = await withdrawalRequests.findOne({ requestId: normalizedRequestId });
    return {
      previousAddress,
      newAddress,
      request: toWithdrawalResponse(updated)
    };
  }
    ...
    try {
      const processed = await runInTransaction(async (session) => {
        ...
        if (targetStatus === 'approved') {
          ...
          // Rejecting is a refund — the user must get their `amount` back no matter
          // what, so unlocking can't be allowed to fail the whole rejection just
          // because the wallet's lockedBalance ledger has drifted out of sync with
          // this specific request. Clamp how much we pull OUT of locked to what's
          // actually sitting there, while still crediting the full amount back to
          // available — otherwise this throws INSUFFICIENT_LOCKED_BALANCE and
          // blocks admin from rejecting an otherwise-valid request.
          const walletForUnlock = await wallets.findOne({ userId: request.userId }, { session });
          const currentlyLocked = resolveLockedBalance(walletForUnlock);
          const lockedDelta = -Math.min(amount, currentlyLocked);
          await applyWalletDeltaInSession({
            session,
            userId: request.userId,
            username: String(actor.username || request.userId).trim(),
            deltaAvailable: amount,
            deltaLocked: lockedDelta,
            amount,
            type: 'refund',
            currency,
          ...
          // Admin rejection (not the user's own cancel) is a fraud/abuse signal:
          // block new P2P sell ads until an admin clears it. Live ads untouched.
          if (isAdminAction && adminUserProfiles) {
            await adminUserProfiles.updateOne(
              { userId: request.userId },
              {
                $set: {
                  sellRestricted: true,
                  sellRestrictedReason: 'withdrawal_rejected',
                  sellRestrictedAt: new Date(),
                  updatedAt: new Date()
                }
              },
              { upsert: true, session }
  ...
  return {
    ...
    updateWithdrawalRequestAddress,
