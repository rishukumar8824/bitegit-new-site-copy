...
const _P2P_BENEFITS_HTML = (function() {
  ...
  return '<div class="p2p-benefit-banner">'
    ...
    +     '</div>'
    +   '</div>'
    +   '<div class="p2p-benefit-img"><div class="bft-svg-card">'
    +     '<svg width="240" height="200" viewBox="0 0 240 200" fill="none" xmlns="http://www.w3.org/2000/svg">'
    +       '<rect width="240" height="200" rx="16" fill="#0d0d0d"/>'
    +       '<rect x="16" y="28" width="72" height="110" rx="10" fill="#1a1a2e" stroke="#F68F15" stroke-width="1.2"/>'
    +       '<rect x="22" y="36" width="60" height="8" rx="4" fill="#F68F15" opacity="0.3"/>'
    +       '<rect x="22" y="50" width="44" height="5" rx="2.5" fill="rgba(255,255,255,0.12)"/>'
    +       '<rect x="22" y="60" width="52" height="5" rx="2.5" fill="rgba(255,255,255,0.08)"/>'
    +       '<rect x="22" y="70" width="38" height="5" rx="2.5" fill="rgba(255,255,255,0.08)"/>'
    +       '<circle cx="52" cy="100" r="14" fill="#F68F15" opacity="0.15"/>'
    +       '<text x="52" y="105" text-anchor="middle" font-size="14" fill="#F68F15" font-family="Arial" font-weight="bold">₿</text>'
    +       '<rect x="152" y="62" width="72" height="110" rx="10" fill="#1a2e1a" stroke="#2ebd85" stroke-width="1.2"/>'
    +       '<rect x="158" y="70" width="60" height="8" rx="4" fill="#2ebd85" opacity="0.3"/>'
    +       '<rect x="158" y="84" width="44" height="5" rx="2.5" fill="rgba(255,255,255,0.12)"/>'
    +       '<rect x="158" y="94" width="52" height="5" rx="2.5" fill="rgba(255,255,255,0.08)"/>'
    +       '<rect x="158" y="104" width="38" height="5" rx="2.5" fill="rgba(255,255,255,0.08)"/>'
    +       '<circle cx="188" cy="134" r="14" fill="#2ebd85" opacity="0.15"/>'
    +       '<text x="188" y="139" text-anchor="middle" font-size="11" fill="#2ebd85" font-family="Arial" font-weight="bold">INR</text>'
    +       '<path d="M96 83 L144 83" stroke="#F68F15" stroke-width="1.5" stroke-dasharray="4 3"/>'
    +       '<polygon points="140,79 148,83 140,87" fill="#F68F15"/>'
    +       '<path d="M144 117 L96 117" stroke="#2ebd85" stroke-width="1.5" stroke-dasharray="4 3"/>'
    +       '<polygon points="100,113 92,117 100,121" fill="#2ebd85"/>'
    +       '<circle cx="120" cy="100" r="18" fill="#111" stroke="#F68F15" stroke-width="1.5"/>'
    +       '<text x="120" y="96" text-anchor="middle" font-size="7" fill="rgba(255,255,255,0.5)" font-family="Arial">P2P</text>'
    +       '<text x="120" y="107" text-anchor="middle" font-size="8" fill="#F68F15" font-family="Arial" font-weight="bold">0%</text>'
    +       '<rect x="50" y="168" width="140" height="22" rx="11" fill="#F68F15"/>'
    +       '<text x="120" y="183" text-anchor="middle" font-size="11" fill="#000" font-family="Arial" font-weight="900" letter-spacing="1">BITCOVEX P2P</text>'
    +     '</svg>'
    +   '</div></div>'
    + '</div>'
...
function renderOffers(data, append) {
  ...
  data.offers.forEach((offer, index) => {
    ...
    const _respMins = offer.responseTime != null ? offer.responseTime : (offer.releaseTime ? Math.max(1, offer.releaseTime) : null);
    const _respLabel = _respMins != null ? (_respMins < 60 ? _respMins + 'm' : Math.round(_respMins/60) + 'h') : null;
    const onlineStatus = isOwnAd ? 'online' : (offer.onlineStatus || (index < 4 ? 'online' : 'offline'));
    const paymentGate = offerPayments.map(m => `<span class="gt-pay">${escapeHtml(m)}</span>`).join('');

    const mobPayRows = offerPayments.map(m => {
      const shortName = m.length > 16 ? m.slice(0,16)+'…' : m;
      return `<span class="bbt-pay-item"><span class="bbt-pay-name">${escapeHtml(shortName)}</span></span>`;
    }).join('');
...
p2pMenuToggle?.addEventListener('click', () => {
  requestAnimationFrame(() => requestAnimationFrame(() => setP2PNavOpen(true)));
});
