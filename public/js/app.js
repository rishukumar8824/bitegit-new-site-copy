...
(function () {
  ...
  // Logged-in state: same token key set by /login and /signup
  ...
  function buildMobileMarket() {
    ...
    function renderRowList(pairsList, loading) {
      // Exact port of the live /markets page table (public/market.html —
      // the real Next.js-rendered page, mobile layout): same column
      // widths (38/31/31%), same "Trading Pair/24h Volume" header, same
      // muted volume sub-line, and the same solid red/green change pill
      // (colors read straight from the compiled tailwind css:
      // --color-fn_red_default:#f24040, --color-green-6d:#2ebe65).
      const table = document.createElement('table');
      table.style.cssText = 'width:100%;border-collapse:collapse;background:#000;table-layout:fixed;';

      const thead = document.createElement('thead');
      thead.innerHTML = `<tr>
        <th style="width:42%;text-align:left;padding:6px 2px 10px 2px;color:#a0a3a7;font-size:12px;font-weight:500;white-space:nowrap;">Trading Pair<span style="font-weight:500;">/24h Volume</span></th>
        <th style="width:29%;text-align:right;padding:6px 2px 10px;color:#a0a3a7;font-size:12px;font-weight:500;white-space:nowrap;">Last Price</th>
        <th style="width:29%;text-align:right;padding:6px 2px 10px 2px;color:#a0a3a7;font-size:12px;font-weight:500;white-space:nowrap;">24h Change</th>
      </tr>`;
      ...
      pairsList.forEach(sym => {
        ...
        const vol = t ? fmtVol(t.quoteVolume) : (loading ? '···' : '—');
        const chg = t ? Number(t.change24h) : 0;
        const chgStr = t ? ((chg >= 0 ? '+' : '') + fmt(chg, 2) + '%') : (loading ? '···' : '—');
        const up = chg >= 0;
        ...
        nameText.style.cssText = 'min-width:0;overflow:hidden;display:flex;flex-direction:column;';
        nameText.innerHTML = `<span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><span style="font-size:14px;font-weight:500;color:#fafafa;">${sym}</span><span style="font-size:12px;font-weight:500;color:#fafafa;opacity:0.4;margin-left:2px;">/USDT</span></span>
          <span style="font-size:12px;color:#a0a3a7;margin-top:2px;">${vol}</span>`;
        nameWrap.appendChild(nameText);
        tdName.appendChild(nameWrap);

        const tdPrice = document.createElement('td');
        tdPrice.style.cssText = 'text-align:right;padding:10px 2px;vertical-align:middle;';
        ...
        tdGain.style.cssText = 'text-align:right;padding:10px 2px 10px 2px;vertical-align:middle;';
  ...
  // ── 7b. LOGGED-IN HOMEPAGE HEADER/DASHBOARD ───────────────────────────────
  function applyLoggedInHomeUI() {
    if (!window._cvxLoggedIn) return;
    if (document.getElementById('cvx-loggedin-done')) return;
    if (!/^\/(index\.html)?$/.test(location.pathname)) return;
    ...
    const logoLink = document.querySelector('header a[href="/"]');
    if (logoLink) {
      logoLink.setAttribute('href', '/p2p#profile');
    ...
    const authBtns = document.getElementById('indexAuthBtns');
    if (authBtns) {
      authBtns.innerHTML = '';
    ...
    dash.innerHTML = `
      <div style="position:relative;">
        ...
        <a href="/p2p.html" style="position:absolute;left:0%;top:68.7%;width:21.8%;height:29.9%;"></a>
